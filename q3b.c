...
int main(int argc, char *argv[]) {
 if (argc != 4) {
        printf("Usage: %s <input_path> <blur_output_path> <edge_output_path>\n", argv[0]);
        return 1;
    }

    const char* input_path = argv[1];
    const char* blur_output_path = argv[2];
    const char* edge_output_path = argv[3];
	...
	for (int i = 0; i <= 30; i++) {
		...
		snprintf(input_filename, 256, "%sa%d.pgm", input_path, i);
		snprintf(output_filename_blur, 256, "%sblurred_%d.pgm", blur_output_path, i);
		snprintf(output_filename_edge, 256, "%sedge_%d.pgm", edge_output_path, i);
...
void openfile(const char* filename, FILE** finput)
{
	...
	scan_result = fscanf(*finput, "%s", header);
    if (scan_result != 1) {  // fscanf returns number of items successfully read
